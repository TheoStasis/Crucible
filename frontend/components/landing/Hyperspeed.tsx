...
// Preset Five: Orange/Red left light trails, Blue/White right light trails, Cyan sticks.
// Tuned to be highly cinematic and slow (speed reduced by ~75%).
const defaultOptions: HyperspeedOptions = {
  ...
  movingAwaySpeed: [15, 20],        // Slowed down from [60, 80]
  movingCloserSpeed: [-30, -45],    // Slowed down from [-120, -160]
...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    // Throw if context is null (e.g. context limit reached or unsupported)
  ...
  initPasses() {
    // Validate WebGL context is still alive before setting up passes
    const gl = this.renderer.getContext();
    if (!gl || !gl.getContextAttributes) {
      throw new Error("WebGL context is no longer valid in initPasses.");
    }
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  useEffect(() => {
    // Graceful WebGL check to prevent crashes in headless/test environments
    try {
      const testCanvas = document.createElement("canvas");
      const gl = (testCanvas.getContext("webgl") || testCanvas.getContext("experimental-webgl")) as WebGLRenderingContext | null;
      if (!gl || typeof gl.getContextAttributes !== "function" || !gl.getContextAttributes()) {
        console.warn("WebGL or its context attributes are not fully supported in this environment. Gracefully disabling Hyperspeed.");
        return;
      }
    } catch (e) {
      console.warn("WebGL availability check failed. Gracefully disabling Hyperspeed.", e);
      return;
    }
    ...
    try {
      ...
      myApp
        ...
        .then(() => myApp.init())
        .catch((err) => {
          console.warn("Failed to load assets or initialize Hyperspeed WebGL canvas:", err);
          if (appRef.current) {
            appRef.current.dispose();
            appRef.current = null;
          }
      ...
      console.warn("Failed to create Hyperspeed App:", err);
      if (appRef.current) {
        appRef.current.dispose();
        appRef.current = null;
      }
  ...
  return <div id="lights" className="w-full h-full opacity-65" ref={hyperspeed}></div>;
};
